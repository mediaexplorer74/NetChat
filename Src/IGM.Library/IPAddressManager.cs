...
namespace IGM.Library
{
  public class IPAddressManager : IIPAddressManager
  {
    ...
    public IEnumerable<string> LoadNetworkInterfaces(bool uniqueNetworks = true)
    {
      ...
      {
        IEnumerable<HostName> source2 = hostNames.Where<HostName>((Func<HostName, bool>) 
            (h => h.IPInformation != null && h.IPInformation?.NetworkAdapter != null && h.Type == (HostNameType)1 
            && ((IEnumerable<string>) IPAddressHelper.PrivateAddresses).SingleOrDefault<string>(
                (Func<string, bool>) (s => s == h.CanonicalName?.Substring(0, s.Length))) != null));
