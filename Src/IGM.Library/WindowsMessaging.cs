      ...
      foreach (string host in this.HostNames)
      {
        ...
        for (int index = 0; index < strArray.Length; ++index)
        {
          ...
          try
          {
            ...
            await Broadcaster.ConnectAsync(new EndpointPair(new HostName(host), (Convert.ToInt32(port) + 1).ToString(), new HostName(this._BroadcastAddress), port));
            this._Writers.Add(new DataWriter(await Broadcaster.GetOutputStreamAsync(new HostName(this._BroadcastAddress), port)));
    ...
    protected override async Task StartListen(bool allAdapters = false)
    {
      ...
      if (allAdapters)
      {
        foreach (string hn in this.HostNames)
        {
          ...
          for (index = 0; index < strArray.Length; ++index)
          {
            ...
            Func<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>, EventRegistrationToken> addMethod = new Func<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>, EventRegistrationToken>(datagramSocket.add_MessageReceived);
            Action<EventRegistrationToken> removeMethod = new Action<EventRegistrationToken>(datagramSocket.remove_MessageReceived);
            WindowsMessaging windowsMessaging = this;
            // ISSUE: virtual method pointer
            TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs> handler = new TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>((object) windowsMessaging, __vmethodptr(windowsMessaging, _Listener_MessageReceived));
            WindowsRuntimeMarshal.AddEventHandler<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>>(addMethod, removeMethod, handler);
      ...
      else
      {
        ...
        for (index = 0; index < strArray.Length; ++index)
        {
          ...
          Func<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>, EventRegistrationToken> addMethod = new Func<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>, EventRegistrationToken>(datagramSocket2.add_MessageReceived);
          Action<EventRegistrationToken> removeMethod = new Action<EventRegistrationToken>(datagramSocket2.remove_MessageReceived);
          WindowsMessaging windowsMessaging = this;
          // ISSUE: virtual method pointer
          TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs> handler = new TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>((object) windowsMessaging, __vmethodptr(windowsMessaging, _Listener_MessageReceived));
          WindowsRuntimeMarshal.AddEventHandler<TypedEventHandler<DatagramSocket, DatagramSocketMessageReceivedEventArgs>>(addMethod, removeMethod, handler);
