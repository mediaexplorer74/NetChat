...
namespace Coding4Fun.Toolkit.Controls
{
  internal static class ButtonBaseHelper
  {
    public static void ApplyTitleOffset(ContentControl contentTitle)
    {
      ...
      ((FrameworkElement)contentTitle).Margin = new Thickness(0.0, top, 0.0, bottom);
    }
    ...
    public static void ApplyForegroundToFillBinding(ContentControl control)
    {
      ...
      if (content.IsTypeOf(typeof (Shape)))
      {
        ...
        ButtonBaseHelper.ResetVerifyAndApplyForegroundToFillBinding(control, target);
      }
      else
      {
      ...
      Shape target)
    {
      if (target == null)
        return;
      bool flag = !(target.ReadLocalValue(Shape.FillProperty) is Brush);
